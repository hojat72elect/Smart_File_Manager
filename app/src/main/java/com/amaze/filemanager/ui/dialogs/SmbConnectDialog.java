...
public class SmbConnectDialog extends DialogFragment {
    ...
    public Dialog onCreateDialog(Bundle savedInstanceState) {
        ...
        SmbDialogBinding binding = SmbDialogBinding.inflate(LayoutInflater.from(context));
        ...
        ip.addTextChangedListener(
                new SimpleTextWatcher() {
                    @Override
                    public void afterTextChanged(@NonNull Editable s) {
                        if (ip.getText().toString().isEmpty()) ipTIL.setError(emptyAddress);
                        else ipTIL.setError("");
                    }
                });
        ...
        if (edit) {
            ...
            try {
                ...
                if (userinfo != null) {
                    String inf = decode(userinfo, kotlin.text.Charsets.UTF_8);
    ...
    private SmbFile createSMBPath(String[] auth, boolean anonymous, boolean disableIpcSignCheck) {
        try {
            ...
            if (!TextUtils.isEmpty(domain)) sb.append(encode(domain + ";", kotlin.text.Charsets.UTF_8));
            if (!anonymous)
                sb.append(encode(auth[1], kotlin.text.Charsets.UTF_8))
                        .append(COLON)
                        .append(encode(auth[2], kotlin.text.Charsets.UTF_8))
                        .append(AT);
    ...
    public interface SmbConnectionListener {
         ...
         *             now. See com.amaze.filemanager.database.UtilsHandler#removeSmbPath(String,
         *             String)
         */
